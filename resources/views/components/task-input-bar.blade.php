            ...
            <div class="flex gap-2 items-center">
                <div class="relative flex-1" @click.outside="showAutocomplete = false">
                    ...
                    <textarea name="name"
                              ...
                              x-on:paste="$nextTick(() => { const el = $refs.createInput; const cleaned = el.value.split('\n').map(l => l.replace(/^[-*] /, '')).join('\n'); if (cleaned !== el.value) { el.value = cleaned; el.dispatchEvent(new Event('input')); } el.style.height = 'auto'; el.style.height = Math.min(el.scrollHeight, 200) + 'px'; })"
