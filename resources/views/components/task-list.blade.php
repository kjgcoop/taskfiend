                ...
                @if($task->assignees->count() > 0)
                    <div class="flex-shrink-0 flex -space-x-1.5">
                        ...
                        @foreach($task->assignees->take(3) as $assignee)
                            <div class="w-7 h-7 rounded-full {{ $avatarColors[$assignee->id % count($avatarColors)] }} flex items-center justify-center text-xs font-bold text-white ring-2 ring-gray-800"
                                 title="{{ $assignee->name }}">
                                {{ strtoupper(substr($assignee->name, 0, 1)) }}
                            </div>
                        @endforeach
                        @if($task->assignees->count() > 3)
                            <div class="w-7 h-7 rounded-full bg-gray-600 flex items-center justify-center text-xs font-medium text-gray-300 ring-2 ring-gray-800"
